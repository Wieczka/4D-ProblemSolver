...
root_agent = Agent(
    ...
    instruction=(
        ...
        "2. Delegate to the `triz_solver` sub-agent using `transfer_to_agent`. Wait for it to transfer control back to you with the 3 TRIZ solutions. Do not try to solve the TRIZ part yourself.\n"
        "3. Delegate to the `alternative_solver` sub-agent using `transfer_to_agent`. Wait for it to transfer control back to you with the 3 alternative solutions. Do not try to generate these patterns yourself.\n"
